...
const EditCategory = () => {
  ...
  // Efekt pobierający dane kategorii z serwera przy montowaniu komponentu
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        const { type, name, amount } = response.data.category;
        setCategoryData({ type, name, amount });
      })
      .catch((error) => {
  ...
  // Obsługa zmiany nazwy kategorii
  ...
  // Obsługa zmiany niestandardowej nazwy kategorii
  const handleCustomNameChange = (e) => {
    setCustomCategoryName(e.target.value);
  };

  // Obsługa zmiany typu kategorii
  ...
  // Obsługa zmiany innych danych kategorii
  const handleChange = (e) => {
    const { name, value } = e.target;
    setCategoryData({ ...categoryData, [name]: value });
  };

  // Obsługa zapisywania zmian kategorii
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:8080/api/categories/edit-category/${categoryId}`,
        dataToSend,
        config
      );
  ...
  return (
    <div className={styles.bodyEditCategory}>
